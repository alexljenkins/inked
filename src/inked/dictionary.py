...
from .utils import random_choice_arr
...
class CharDict(object):
    ...
    def __init__(
        ...
        self.dictionary = self.__load_possible_words(possible_chars)
        self.possible_chars = list(set([item for sublist in self.dictionary.values() for item in sublist]))
    ...
    def random_char(self) -> str:
        ...
        if len(self.possible_chars) == 0:
            raise KeyError
        return np.random.choice(self.possible_chars)

    def __load_possible_words(self, possible_chars: Set[str]) -> Dict[str, Dict[str, List[str]]]:
        ...
        return {
            k: index_by_char(supported(v, possible_chars))
            for k, v in full_dictionary.items()
            if len(supported(v, possible_chars)) > 0
        }
    ...
    def __getitem__(self, key: str) -> str:
        ...
        return random_choice_arr(self.dictionary[rand_dict][key])
